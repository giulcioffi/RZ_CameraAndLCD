...
#include    "DisplayBase/lvds_pll_calc.h"

/******************************************************************************
Macro definitions
******************************************************************************/
/* FIN: LVDS PLL input frequency */
#define     FIN_MIN             (9.6)           /*!< The lower limit of the FIN */
#define     FIN_MAX             (87.0)          /*!< The upper limit of the FIN */
/* FREF */
#define     FREF_MIN            (9.6)           /*!< The lower limit of the FREF */
#define     FREF_MAX            (13.0)          /*!< The upper limit of the FREF */
/* FVCO: VCO output frequency */
#define     FVCO_MIN            (580.0)         /*!< The lower limit of the FVCO */
#define     FVCO_MAX            (1250.0)        /*!< The upper limit of the FVCO */
/* FOUT: LVDS PLL output frequency */
#define     FOUT_MIN            (290.0)         /*!< The lower limit of the FOUT */
...
static void SeekValue (pll_parameter_t * param)
{
    ...
    dcdr_list = dcdr_data;
    while (*dcdr_list != 0.0) {
        fnrdin = InputClock / *dcdr_list;

        nidiv_list = NIDIV_data;
        while (*nidiv_list != 0.0) {
            fin = fnrdin / *nidiv_list;
            ...
            while (*nrd_list != 0.0) {
                ...
                nfd_list = NFD_data;
                while (*nfd_list != 0.0) {
                    fvco = fin * (*nfd_list) * NOD_data[0] / (*nrd_list);
                    if ((fvco < FVCO_MIN) || (FVCO_MAX < fvco)) {
                        nfd_list++;
                        continue;
                    }

                    fout = fin * (*nfd_list) / (*nrd_list);
